...
export function Markdown({ content, change, view, permission }) {
  ...
  const MarkdownComponents = {
    ...
    li: ({ children }) => (
      <li style={{ marginBottom: "0.75em", color: "#555" }}>{children}</li>
    ),
    ...
    tr: ({ children }) => (
      <tr
        style={{ borderBottom: "1px solid #ddd", backgroundColor: "#f9fafb" }}
      >
        {children}
      </tr>
    ),
    ...
    span: ({ children }) => <span style={{ color: "#333" }}>{children}</span>,

    strong: ({ children }) => (
      <strong style={{ fontWeight: "bold", color: "#000" }}>{children}</strong>
    ),
  };
