...
export default function Dashboard() {
  ...
  const handleUserClick = async (id) => {
    ...
    const resSelectUser = await fetch(
      `/api/db?table=admin_selectUser&userId=${selectedUserId}`,
    );
        ...
        <div className={styles.usersContent}>
          {modalUser && (
            <AdminUserInfo
              ...
              toggleModalAdminUserInfo={toggleModalAdminUserInfo}
            />
          )}
          <input
            type="search"
            ...
            <tbody>
              {filteredUsers.map((user) => (
                <tr key={user.id} onClick={() => handleUserClick(user.id)}>
                  <td
                    style={
                      user.delete ? { textDecoration: "line-through" } : {}
                    }
                  >
                    {user.id}
                  </td>
            ...
            <tbody>
              {filteredGroups.map((group) => (
                <tr key={group.id}>
                  <td
                    style={
                      group.delete ? { textDecoration: "line-through" } : {}
                    }
                  >
                    {group.id}
                  </td>
            ...
            <tbody>
              {filteredRoles.map((role) => (
                <tr key={role.id}>
                  <td
                    style={
                      role.delete ? { textDecoration: "line-through" } : {}
                    }
                  >
                    {role.id}
                  </td>
